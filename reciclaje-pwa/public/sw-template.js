...
const apiUrlsToCache = [
  `${API_BASE_URL}/api/puntos`,
  `${API_BASE_URL}/api/productos`,
  `${API_BASE_URL}/api/reciclajes/historial`
];
...
self.addEventListener('fetch', (event) => {
  ...
  if (request.method === 'GET' && !url.origin.includes(API_BASE_URL.replace('http://', '').replace('https://', ''))) {
    event.respondWith(
  ...
  else if (request.method === 'GET' && url.origin.includes(API_BASE_URL.replace('http://', '').replace('https://', ''))) {
    event.respondWith(
      fetch(request)
        .then((response) => {
